(ns foundation.app.test.tree
  (:require [cemerick.cljs.test :as t]
            [foundation.app.messages :as msg]
            [foundation.app.tree :as tree :refer [map->deltas]]
            [foundation.app.query :refer [q]])
  (:require-macros [cemerick.cljs.test :refer [deftest run-tests is testing]]))

(defn test-apply-deltas [ui d]
  (into {} (tree/apply-deltas ui d)))

(defn isomorphic [ui d]
  (into {} (-> ui
               (tree/apply-deltas d)
               (tree/apply-deltas (tree/invert d)))))

(deftest test-real-path
  (is (= (tree/real-path []) [:tree]))
  (is (= (tree/real-path [:a]) [:tree :children :a]))
  (is (= (tree/real-path [:a :b]) [:tree :children :a :children :b]))
  (is (= (tree/real-path [:a :b 2]) [:tree :children :a :children :b :children 2])))

(deftest test-node-enter
  (testing "add a single node"
    (let [ui tree/new-app-model
...
(deftest test-node-exit
  (let [ui tree/new-app-model
        ...
        delta-one
        {0 [{:delta [:node-create [] :map] :seq 0 :t 0}
            {:delta [:node-create [:a] :map] :seq 1 :t 0}
               ...
               {:deltas
                (assoc delta-one 1
                       [{:delta [:value [:a :b :c 1] 3 nil] :seq 14 :t 1}
                        ...
                        {:delta [:transform-disable [:a :b :c 0] :f [{:x :p}]]
                         :seq 18 :t 1}
...
(deftest test-node-isomorphism
  (let [ui tree/new-app-model
        deltas [[:node-create [] :map]
                ...
                [:node-create [:a :b] :vector]
                [:node-create [:a :b 0] :map]]
        inverse (map tree/inverse (reverse deltas))]
    (is (= inverse
...
(deftest test-remove-index-from-vector
  (is (= (tree/remove-index-from-vector [0 1 2 3] 0)
         [1 2 3]))
  (is (= (tree/remove-index-from-vector [0 1 2 3] 1)
         [0 2 3]))
  (is (= (tree/remove-index-from-vector [0 1 2 3] 2)
         [0 1 3]))
  (is (= (tree/remove-index-from-vector [0 1 2 3] 3)
         [0 1 2])))
...
(deftest test-since-t
  (let [deltas {0 [{:delta [:node-create [] :map] :seq 0 :t 0}
    ...
    (is (= (tree/since-t {:deltas deltas :t 5} 0)
           (map :delta (apply concat (vals deltas)))))
    (is (= (count (tree/since-t {:deltas deltas :t 5} 1))
           5))
    (is (= (count (tree/since-t {:deltas deltas :t 5} 3))
           3))))
    ...
    (let [deltas [[:children-exit [:a :c]]]
      ...
      (is (= (:tree test-tree)
             {:children {:a {:children {:b {:children [{:children {}}
                                                       {:children {}}]}
                                        :c {:children {}}}}}}))
      (is (= (set (tree/since-t test-tree 0))
             (set (map :delta
                       (apply concat
...
(deftest test-transform->entities
  (reset! tree/next-eid-atom 10)
  (is (= (tree/transform->entities :navigate
...
(deftest test-transforms->entities
  (reset! tree/next-eid-atom 10)
  (let [result (tree/transforms->entities
                {:navigate [{:page :page/configuration}
                            {msg/topic :y :style :awesome}]
...
(deftest test-attrs-entities
  (reset! tree/next-eid-atom 10)
  (is (= (tree/attrs->entities {:color :red :size 10} 1)
...
(deftest test-tree->entities
  (let [tree->entities tree/tree->entities]
    (let [ui tree/new-app-model
          deltas [{:a {:value 42
                       :attrs {:color :red :size 10}
                       :transforms {:x [{:y :z}]}
...
(deftest test-entity->tuples
  (let [entity->tuples tree/entity->tuples]
    (is (= (entity->tuples {:t/id 7 :color :red :size 10})
           [[7 :color :red]
            [7 :size 10]]))))

(deftest test-tree->tuples
  (let [tree->tuples tree/tree->tuples]
    (let [ui tree/new-app-model
          deltas [{:a {:value 42
                       :attrs {:color :red :size 10}
                       :transforms {:x [{:y :z}]}
...
(def test-tree
  (tree/apply-deltas tree/new-app-model deltas))

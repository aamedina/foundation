(ns foundation.app.test.query
  (:require [foundation.app.query :as query :refer [q]]
            [cemerick.cljs.test :as t])
  (:require-macros [cemerick.cljs.test :refer [deftest run-tests is testing]]))

(deftest test-unifier
  (is (= (query/unifier {} '[?p :name ?parent] [1 :name :john])
         '{?p 1 ?parent :john}))
  (is (= (query/unifier {} '[?p :age ?parent] [1 :name :john])
         nil)))

(deftest test-combine-unifiers
  (is (= (query/combine-unifiers '[{?p-name :bob ?p 1}
                             {?p-name :sam ?p 2}
                             {?p-name :dave ?p 3}
                             {?p-name :alice ?p 4}]
                           '[{?c 1}])

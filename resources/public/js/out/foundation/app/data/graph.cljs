...
(defn add-loops
  ...
  (DirectedGraph.
   (:nodes g)
   (into {}
         (map (fn [n] [n (conj (set (get-neighbors g n)) n)]) (:nodes g)))))
...
(defn lazy-walk
  ...
  ([g ns v]
     (lazy-seq (let [s (seq (drop-while v ns))
                     ...
                     ns (rest s)]
                 (when s
                   (cons n (lazy-walk g (concat (get-neighbors g n) ns)
                                      (conj v n))))))))
...
(defn transitive-closure
  ...
  (let [nns (fn [n]
    ...
    (DirectedGraph. (:nodes g) (fn [n] (force (nbs n))))))

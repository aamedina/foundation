...
(ns foundation.app.data.dependency
...
(deftype MapDependencyGraph [dependencies dependents]
  ...
  (depend [graph node dep]
    (when (or (= node dep) (depends? graph dep node))
      (throw (js/Error. (str "Circular dependency between "
  ...
  (remove-node [graph node]
    (MapDependencyGraph.
     ...
     dependents))
  IPrintWithWriter
  (-pr-writer [x opts writer]
    (-pr-writer {:dependencies (.-dependencies x)
                 :dependents (.-dependents x)} opts writer)))

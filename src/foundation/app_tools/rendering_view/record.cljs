...
(ns foundation.app-tools.rendering-view.record
...
(defn save-recording [recording]
  (js/console.log (pr-str recording))
  (let [recording-name (get-in recording [:config :name])]
    
    (xhr/POST
     (str "http://localhost:3000/_tools/render/recordings/"
          (name recording-name))
    ...
    (when (not recording?)
      (d/generic-modal-collect-input "content"
                                     ...
                                     [{(msg/param :name) {}
                                       ...
                                       (msg/param :order) {}}]))
    (take! queue #(make-and-save-recording %))))

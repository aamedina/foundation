...
(ns foundation.app.render.automatic
  (:require [foundation.app.render :as r]
            ...
            [foundation.app.render.events :as events]))
  ...
  (let [syms (msg/message-params messages)]
    (when (seq syms)
      (let [modal-id (modal-id id transform-name)
        ...
        (str "<div class='modal fade' id='" modal-id "' tabindex='-1' role='dialog'"
...
(defn render-node-enter [r [_ path] input-queue]
  (let [parent (render/get-parent-id r path)
    ...
    (when-not (d/has-class? (d/by-id "content") "container")
      (d/add-class! (d/by-id "content") "container"))
    (d/append! (d/by-id parent)
               (str "<div id='" id "' class='" (if (<= path-length 1)
                                                 "root-node-section"
                                                 "node-section") "'>"
                    "  <div class='row'>"
                    "    <div class='col-sm-3' style='text-align:right' id='" control-id "'></div>"
                    "    <div class='col-sm-9'>"

...
(ns foundation.app.render.events
...
(defn send-transforms
  ...
  ([input-queue transform-name messages]
     (send-transforms input-queue
                      (map (partial msg/add-message-type transform-name)
                           messages)))
...
(defn collect-and-send
  [event-type dc input-queue transform-name messages input-map]

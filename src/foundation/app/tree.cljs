...
(defn- map->deltas [tree path]
  (let [node-keys #{:children :transforms :value :attrs}
        node? (and (map? tree) (not (empty? (set/intersection (set (keys tree))
                                                              node-keys))))
...
(defn transform->entities [transform-name msgs node-id]
  (let [transform-id (next-eid)]
    (concat [{:t/id transform-id :t/transform-name transform-name :t/node node-id :t/type :t/transform}]
            (map (fn [m] (merge m {:t/id (next-eid) :t/transform transform-id
...
(defn attrs->entities [attrs node-id]
  (when (not (empty? attrs)) [(merge attrs {:t/id (next-eid) :t/node node-id
                                            :t/type :t/attrs})]))
...
(defn delete-deltas [t deltas]
  (reduce (fn [d k]
            (if (< k t)
              (do (log/debug :gc (str "GC: Deleting "
                                      (count (get d k)) " deltas at time " k))
                  (dissoc d k))
              d))
          deltas
          (keys deltas)))
